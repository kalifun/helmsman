...
- [ ] P1·策略学习·命令级白名单（rm/sudo/git push 精确匹配 + 拒绝幂等 —— 待引擎工具审批缝）
- [ ] P2·向量检索（本地 embedding：规则检索已跑基线，向量未上）
- [ ] P2·知识演化（毒化检测/失效已有；自动提炼/合并未做）
- [ ] P2·桌面壳（Tauri v2 sidecar）
- [ ] P2·文件内容预览（FilesView 现只有目录树）
- [ ] P2·设置热更新（改 dsh 配置需重启引擎）
- [ ] P2·共享工作区写冲突检测
